...
def current_portfolio_stats(holdings: pd.DataFrame, mean_returns, cov_matrix, tickers: list) -> dict | None:
...
# REPORTING
# ─────────────────────────────────────────────
def print_report(max_sharpe, min_var, current, tickers, mean_returns, cov_matrix):
    div = "─" * 55
    ...
    if current:
        ...
        if sharpe_gap > 0.1:
            print(f"\n  💡  Optimizing to Max Sharpe could improve your Sharpe by +{sharpe_gap:.2f}")
...
def main():
    ...
    if holdings is not None:
        current = current_portfolio_stats(holdings, mean_returns, cov_matrix, tickers)
        if current:
    ...
    # ── Print report ──
    print_report(ms, mv, current, tickers, mean_returns, cov_matrix)

    # ── Charts ──
    if not args.no_plot:
        plot_efficient_frontier(
            mc_df, frontier_df, ms, mv, current, tickers,
